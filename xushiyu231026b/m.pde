class Particle {
  ...
  void display() {
    ...
    for(int i = 1; i < trace.size(); i ++){
      float sc = map(i, 0, trace.size()-1, 0, 255);
      stroke(sc);
      float sw = map(i, 0, trace.size()-1, 5, 0.1);
      strokeWeight(sw);
      ellipse(trace.get(i-1).x, trace.get(i-1).y, trace.get(i).x*0.05, trace.get(i).y*0.05);
    }
  }
  
  void squiggly(){
    float theta = noise(pos.x*squiggliness, pos.y*squiggliness)*PI*noiseFactor;//矩阵相邻相近
